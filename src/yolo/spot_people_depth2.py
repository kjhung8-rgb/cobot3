"""
spot_people_depth2.py
Spot 로봇 카메라로 사람을 YOLO 감지하고 depth 기반 Spot 카메라 상대 좌표를 출력.
...
SPOT_POS = np.array([0.0, 0.75, 0.65])  # Spot body 월드 위치

def pixel_to_spot_relative(u, v, depth_val, cam_pos):
    x_c = (u - CX) * depth_val / FX
    y_c = (v - CY) * depth_val / FY
    z_c = depth_val
    world = cam_pos + R_CAM_TO_WORLD @ np.array([x_c, y_c, z_c])
    return world - SPOT_POS  # Spot body 기준 상대 좌표
CAPTURE_STEP  = 120
...
while simulation_app.is_running():
    ...
    for i, box in enumerate(boxes):
        ...
        if d > 0:
            sp = pixel_to_spot_relative(uc, vc, d, cam_pos)
            coord_str = f"({sp[0]:.2f}, {sp[1]:.2f}, {sp[2]:.2f})m"
        else:
            coord_str = "(depth 없음)"

        print(f"  [{i+1}] conf={conf:.2f}  depth={d:.2f}m  spot_rel={coord_str}")

        cv2.rectangle(vis, (x1, y1), (x2, y2), (0, 255, 0), 2)
        cv2.putText(vis, f"person {conf:.2f}",
                    (x1, y1 - 8), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 255, 0), 2)
        cv2.putText(vis, coord_str,
                    (x1, y2 + 20), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (255, 200, 0), 2)
