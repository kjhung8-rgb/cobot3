...
def _x_marker_msg(pose: PoseStamped, half: float, z_lift: float, line_width: float) -> Marker:
...
def _sphere_marker_msg(pose: PoseStamped, diameter: float, z_lift: float) -> Marker:
    m = Marker()
    m.header = pose.header
    m.ns = "survivor"
    m.id = 1
...
class SurvivorPoseToMarker(Node):
    ...
    def _cb(self, msg: PoseStamped):
        self._pub.publish(_x_marker_msg(msg, self._half, self._z_lift, self._line_width))
        self._pub.publish(_sphere_marker_msg(msg, self._sphere_diameter, self._z_lift))

        if self._log_each:
            p = msg.pose.position
            o = msg.pose.orientation
            self.get_logger().info(
                f"[survivor pose] frame={msg.header.frame_id} "
