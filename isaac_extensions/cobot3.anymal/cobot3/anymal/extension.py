"""
Cobot3 ANYmal C Extension
==========================

Thin UI entrypoint. Heavy logic is split into:
  - scene.py            : AnymalNavScene world/policy runtime
  - ros_graphs.py       : cmd_vel, camera, LiDAR/SLAM graph setup
  - teleop_launcher.py  : terminal launcher for anymal_teleop.py
  - constants.py        : /anymal_0 topic/frame contract
"""
...
from .scene import AnymalNavScene
from .teleop_launcher import TeleopLauncher
from .utils import get_ros_domain_id


class Cobot3AnymalExtension(omni.ext.IExt):
    """Isaac Sim extension UI for the Cobot3 ANYmal C Nav2 project."""

    def on_startup(self, ext_id):
        print("[cobot3.anymal] Extension 시작")
        self._sample = None
        self._teleop = TeleopLauncher()

        self._window = ui.Window("Cobot3 ANYmal C - Nav2", width=440, height=520)
        with self._window.frame:
            with ui.VStack(spacing=6):
                ui.Label("ANYmal C + Nav2 장애물회피", style={"font_size": 16})
        ...
        print("[cobot3.anymal] UI 준비 완료")

    def on_shutdown(self):
        print("[cobot3.anymal] Extension 종료")
        if hasattr(self, "_teleop") and self._teleop:
            self._teleop.stop()
        self._window = None

    def _load_scene(self):
        self._sample = AnymalNavScene()
        asyncio.ensure_future(self._sample.load_world_async())
        print("[cobot3.anymal] Scene 로딩 중... (잠시 기다려주세요)")
